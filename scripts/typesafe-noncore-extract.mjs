/** Native extraction restricted to three existing Beatboxset validation recordings. */
import { chromium } from '@playwright/test';
import { readFileSync, writeFileSync } from 'node:fs';
const directory='artifacts/typesafe-noncore';
const manifest=JSON.parse(readFileSync(`${directory}/audio-manifest.json`,'utf8'));
const allowed=new Set(manifest.rows.map(r=>`${r.stem}-audio.f32`));
if(manifest.rows.length!==3 || manifest.rows.some(r=>!['putfile_bui','putfile_dbztenkaichi','putfile_pepouni'].includes(r.stem)))throw Error('Validation allowlist mismatch');
...
for(const row of manifest.rows){
 const result=await page.evaluate(async row=>{
  ...
  const samples=new Float32Array(await(await fetch(`/browser-relative-test-fixtures/${row.stem}-audio.f32`)).arrayBuffer());
