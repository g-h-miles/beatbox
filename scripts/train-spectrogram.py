...
if Path('artifacts/spectrograms.npy').exists():X=np.load('artifacts/spectrograms.npy')
else:
 ...
 for i,r in enumerate(rows):
  ...
  a=cache[path];start=int(r['time']*sr);end=min(len(a),int((r['time']+r['duration'])*sr));clip=np.zeros(nfft+hop*(frames-1));length=min(end-start,len(clip));clip[:length]=a[start:start+length]
 ...
 np.save('artifacts/spectrograms.npy',X)
...
for epoch in range(50):
 ...
 if score>best:
  best=score;torch.save(model.cpu().state_dict(),'artifacts/spectrogram-model.pt');model.to(device)
model.load_state_dict(torch.load('artifacts/spectrogram-model.pt',weights_only=True));test=p>=21;pred=predictions(test);print('FINAL TEST',accuracy_score(y[test],pred),'GROOVES',accuracy_score(y[test&improv],pred[improv[test]]),confusion_matrix(y[test],pred).tolist(),flush=True)
