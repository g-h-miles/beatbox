/** Frozen production TypeSafe versus frozen relative SVM on AVP validation15–20. */
...
const records = JSON.parse(readFileSync('artifacts/events-v2.json', 'utf8')).filter((r: any) => r.participant >= 15 && r.participant <= 20 && r.file.includes('Improvisation'));
...
for (const record of records) {
  ...
  for (const candidate of candidates) {
    ...
    const hybrid = ['ride', 'crash', 'aux'].includes(typeSafe) ? typeSafe : svm === 'hat'
...
const report = { date: new Date().toISOString(), scope: 'Frozen AVP validation performers15–20 Fixed/Personal; zero examples, no fitting or test voices.',
  features: 'Original44.1kHz basicfeatures+spectrum; fullneuralcrop up to0.5s, fixed active duration. No texture mask.',
  reused: 'P15/P16 neuralActive API responses reused unchanged; remaining8recordings classified once.', summaries,
  hybrid: 'Predeclared before new scores: retain TypeSafe ride/crash/aux; otherwise SVM core class; hat subtype by larger TypeSafe closed/open posterior. No invented SVM probabilities. Noncore retention is not noncore validation.',
  recordings: results.map(({ hits, ...r }) => r), limitation: 'Previously inspected validation cohort, not independent final testing. TypeSafe seven output labels; SVM core three labels.' };
