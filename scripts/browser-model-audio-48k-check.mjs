...
const result = await page.evaluate(async (fixtures) => {
  ...
  for (const fixture of fixtures) {
    ...
    const source = await load(`${fixture.stem}-native.f32`);
    const offline = new OfflineAudioContext(1, Math.ceil(source.length * 48000 / fixture.sampleRate), 48000);
    const input = offline.createBuffer(1, source.length, fixture.sampleRate);
    input.copyToChannel(source, 0);
    const playback = offline.createBufferSource(); playback.buffer = input; playback.connect(offline.destination); playback.start();
    const original = (await offline.startRendering()).getChannelData(0).slice();
    ...
    if (fixture.split === "validation") {
      const neural = await detectNeural(original, 48000);
  ...
  return {
    protocol:
      "Original raw coefficient parity plus new frozen browser-trained worker validation after browser resampling to48kHz; no new fitting or selection.",
    results,
    milliseconds100,
...
await browser.close();
writeFileSync(
  "artifacts/browser-model-parity/audio-48k-report.json",
