...
def main():
 ...
 for rec in manifest['recordings']:
  audio,rate=sf.read(a.manifest.parent/rec['file'],dtype='float32')
  segs=segment_isolated(audio,rate)
  for candidate in segment_isolated(audio,rate,minimum_relative_peak=0):
  ...
  for i,seg in enumerate(segs):
   ...
   resampled=librosa.resample(sample,orig_sr=rate,target_sr=16000)
   ...
   rows.append({'file':rec['file'],'label':rec['drum'],'split':rec['split'],'fold':min(2,int(i*3/len(segs))),**seg})
 ...
 (a.manifest.parent/'balanced-descriptor-assessment.json').write_text(json.dumps(report,indent=2));print(json.dumps({k:v for k,v in report.items() if k not in ['candidates','events','excluded']}))
 np.savez_compressed(a.manifest.parent/'balanced-descriptor-profile.npz',X=X[best['feature']][train],y=y[train])
