...
## Browser trained candidate and groove control visibility

The locally staged browser trained classifier passed 55 tests and a production build. The full browser flow loaded both new model binaries, preserved manual timing and velocity edits, kept all displayed hit timestamps unchanged, and recovered from deliberate model and API failures. Real production API calls were used for the primary flow. Evidence is saved separately under `artifacts/browser-trained-ui/`.

Groove hints now appears only when the raw results include an unconfirmed TypeSafe kick or snare with probabilities. The disclosure is hidden after successful local model classification and returns for eligible TypeSafe fallback results. The tempo field remains visible. Neither the hint preference nor timing/rhythm logic changed. A separate explicit API fixture run verified that an unchecked preference stays unchecked across hiding and showing the disclosure; model inference remained real. That fixture run is evidence of UI state behavior, not real API or recognition accuracy, and is saved under `artifacts/browser-trained-groove-fixture/`.

## Deployed browser trained models smoke check

The true production smoke completed on 2026-09-17 at 05:21 UTC after main `15be76a` and deployment `e33cf29e-ca51-485a-b713-6168093109ed`. It used no request interception, mocks, or proxy. The browser loaded `worker-wdbCfaSA.js`, `model3-T2VNkSnR.bin`, `model4-DkaVnCYX.bin`, the onset model, and the MJS/WASM runtime with HTTP 200. Both real classification requests also succeeded.

Public P15 Personal improvisation produced 27 hits. The manually edited snare and velocity 101 survived classification, and all displayed onset times stayed unchanged. Groove hints was hidden for the model results while Tempo remained available. Original playback, drum preview, and download worked. Parsed MIDI contained 27 notes on channel 10 at 9,600 PPQ, with edited snare note 38 and velocity 101 preserved. Its maximum difference from rounded UI timestamps was 0.5104 ms. No JavaScript page errors or horizontal overflow occurred, and the final desktop screenshot was visually reviewed.

Separate evidence is in `artifacts/browser-trained-live/report.json`, `live.png`, and `reviewed.mid`. This confirms deployed functionality and export consistency; it does not add a classification accuracy claim.
