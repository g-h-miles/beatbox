...
export function coreDiversity(spectra: (number[] | undefined)[]): number {
  if (
    spectra.length < 6 ||
    spectra.some(
      (s) => !s || s.length !== 20 || s.some((v) => !Number.isFinite(v)),
    )
  )
    return 0;
  ...
  const selected = Array.from(
    { length: count },
    (_, i) => spectra[Math.floor((i * spectra.length) / count)]!,
  );
  ...
  for (let i = 0; i < count; i++)
    for (let j = i + 1; j < count; j++) {
      let square = 0;
      for (let k = 0; k < 20; k++)
        square += (selected[i][k] - selected[j][k]) ** 2;
      distances.push(Math.sqrt(square / 20));
    }
  distances.sort((a, b) => a - b);
  return distances[Math.floor((distances.length - 1) * 0.75)];
}
